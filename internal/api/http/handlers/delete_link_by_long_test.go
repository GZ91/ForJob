...
func Test_handlers_DeleteLinkByLongLink(t *testing.T) {
	...
	r := chi.NewRouter()
	r.HandleFunc("/links/long", h.DeleteLinkByLongLink)
	...
	testCases := []TestCase{
		{
			...
			funcMock: func() {
				mockStorager.On("DeleteLinkByLongLink", mock_test.Anything, "longURL", "existent_token1").Return(errors.New("other"))
			},
		},
		{
			name:         "status ok",
			...
			funcMock: func() {
				mockStorager.On("DeleteLinkByLongLink", mock_test.Anything, "longURL", "existent_token2").Return(nil)
			},
		},
		{
			name:         "status ok",
			...
			funcMock: func() {
				mockStorager.On("DeleteLinkByLongLink", mock_test.Anything, "longURL", "existent_token3").Return(errorsapp.ErrNotFoundLink)
			},
		},
	}
